...
from forge.core.domain.models import TaskStatus
...
def init() -> None:
    ...
    try:
        ...
        console.print(
            f"[bold green][OK][/bold green] Database initialized at: [cyan]{settings.db_url}[/cyan]"
        )
        console.print(
            f"[bold green][OK][/bold green] Plugins directory configured: [cyan]{settings.plugins_dir}[/cyan]"
        )
        console.print(
            '[bold green]Forge is ready to run! Use: [cyan]forge run "your goal"[/cyan][/bold green]'
        )
...
def run(
    ...
    async def _run() -> None:
        ...
        with Live(
            render_live_panel(goal, "pending", execution.tasks), refresh_per_second=4
        ) as live:
...
def resume(
    ...
    async def _resume() -> None:
        ...
        with Live(
            render_live_panel(execution.goal, "in_progress", execution.tasks), refresh_per_second=4
        ) as live:
            while not exec_task.done():
                await asyncio.sleep(0.25)
...
def logs(
    ...
    async def _logs() -> None:
        ...
        for entry in logs_list:
            level_color = (
                "red"
                if entry.level.value == "error"
                else "yellow"
                if entry.level.value == "warning"
                else "white"
            )
            table.add_row(
...
def explain(
    ...
    async def _explain() -> None:
        ...
        console.print(
            Panel(summary_text, title=f"Explanation: {execution_id}", border_style="cyan")
        )
        await container.close()
...
def doctor(
    fix: bool = typer.Option(
        False, "--fix", help="Attempt to auto-repair minor configuration issues"
    ),
    ...
    try:
        ...
        if llm_avail:
            ...
            console.print(
                "  State:    [yellow]Offline or Not Reachable (Ollama requires local running instance)[/yellow]"
            )
    except Exception as exc:
    ...
    for binary in ["git", "docker"]:
        ...
        if bin_path:
            ...
            console.print(
                f"  {binary}: [yellow]Not Found on PATH[/yellow] (related executors will fail)"
            )
...
def trace(
    ...
    async def _trace() -> None:
        ...
        if not spans:
            console.print(
                "[yellow]No telemetry trace spans recorded for this execution run.[/yellow]"
            )
            await container.close()
            return

        console.print(
            f"[bold cyan]Performance Trace Timeline for Goal:[/bold cyan] {execution.goal}\n"
        )
        ...
        for idx, span in enumerate(spans):
            ...
            table.add_row(span["span"], f"{dur:.2f} ms", f"[{bar}]")
...
def graph(
    ...
    async def _graph() -> None:
        ...
        for t in sorted_tasks:
            ...
            console.print("     ▼")
            console.print(
                f"  [Task {t.order_index}] [bold]{t.name}[/bold] ({t.task_type.value}) -> [{color}]{t.status.value}[/{color}]"
            )
            if t.dependencies:
                dep_indices = []
...
def install_plugin(
    path: Path = typer.Argument(
        ..., help="Path to local plugin directory containing forge_plugin.json"
    ),
) -> None:
    ...
    async def _install() -> None:
        ...
        try:
            ...
            console.print(
                f"[bold green]Successfully installed plugin: {manifest.name} v{manifest.version}[/bold green]"
            )
        except Exception as exc:
...
def create_plugin(
    ...
    try:
        ...
        console.print(
            f"[bold green]Scaffolded plugin '{name}' template at: [cyan]{plugin_path}[/cyan][/bold green]"
        )
    except Exception as exc:
